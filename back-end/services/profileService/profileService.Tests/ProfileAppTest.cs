...
namespace profileService.Tests
{
    ...
    public class ProfileAppTest
    {
        ...
        {
            Guid userId = Guid.NewGuid();
            ProfileData profileData = new ProfileData { Username = "John Doe", Biography = "hi" };
        ...
        {
            List<ProfileData> expectedProfiles = new List<ProfileData>
            {
                new ProfileData { OwnerId = Guid.NewGuid(), Username = "John Doe", Biography = "hi" },
                new ProfileData { OwnerId = Guid.NewGuid(), Username = "Jane Doe", Biography = "bye" }
            };

            profileRepositoryMock.Setup(x => x.GetAll()).ReturnsAsync(expectedProfiles);

            Task<List<ProfileData>> actualProfilesTask = profileApp.GetAll();

            profileRepositoryMock.Verify(x => x.GetAll(), Times.Once);

            List<ProfileData> actualProfiles = actualProfilesTask.Result;
